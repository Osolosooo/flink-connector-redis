...
public class RedisHandlerServices<T> {
    ...
    private static <T> List<T> filterByFactoryClass(Class<T> redisClass, List<RedisHandler> redis) {
        ...
        List<RedisHandler> redisList = redis.stream().filter(p -> redisClass.isAssignableFrom(p.getClass())).collect(Collectors.toList());
    ...
    private static <T extends RedisHandler> List<T> filterByContext(
        ...
        List<T> matchingredis = redisList.stream().filter(factory -> {
                    ...
                    return plainContext.keySet().stream().allMatch(e -> meta.containsKey(e) && meta.get(e).equals(plainContext.get(e)));
                })
